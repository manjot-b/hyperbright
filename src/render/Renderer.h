...
class Renderer
{
	public:
		...
		public:
			IRenderable() {}
			IRenderable(const std::shared_ptr<openGLHelper::Shader>& shader) : _shader(shader) {}
			virtual void render() const = 0;
			const std::shared_ptr<openGLHelper::Shader>& getShader() const { return _shader; }
			void setShader(const std::shared_ptr<openGLHelper::Shader>& shader) { _shader = shader; }
		protected:
